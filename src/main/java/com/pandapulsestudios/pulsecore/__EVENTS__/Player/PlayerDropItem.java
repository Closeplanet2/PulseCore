    ...
    public void onEvent(PlayerDropItemEvent event){
        ...
        for(var itemStack : inventoryItems.keySet()){
            for(var pulseEnchantment : EnchantmentAPI.ReturnCustomEnchantmentOnItems(itemStack)){
                var state = pulseEnchantment.PlayerDropItemEvent(event, itemStack, inventoryItems.get(itemStack));
        ...
        for(var itemStack : inventoryItems.keySet()){
            ...
            var state = pulseItemStack.PlayerDropItemEvent(event, itemStack, inventoryItems.get(itemStack));
            if(!event.isCancelled()) event.setCancelled(state);
        }
        ...
        for(var pulseLocation :  LocationAPI.ReturnAllPulseLocations(eventLocation, true)){
            var state = pulseLocation.PlayerDropItemEvent(event, eventLocation);
            if(!event.isCancelled()) event.setCancelled(state);
        }
    }
}
