...
import com.pandapulsestudios.pulsecore.Events.CustomEvents.DirCreatedEvent;
import com.pandapulsestudios.pulsecore.Events.CustomEvents.DirDeletedEvent;
...
public class DirAPI {
    ...
    public static void DeleteDirectory(File directory){
        if(!directory.exists()) return;
        var dirDeletedEvent = new DirDeletedEvent(directory);
        if(dirDeletedEvent.isCancelled()) return;
        directory.delete();
    }

    public static void CreateDirectory(File directory){
        if(directory.exists()) return;
        var dirCreatedEvent = new DirCreatedEvent(directory);
        if(dirCreatedEvent.isCancelled()) return;
        directory.mkdirs();
    }
