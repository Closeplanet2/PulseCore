    ...
    public void onEvent(PlayerItemBreakEvent event){
        ...
        for(var itemStack : inventoryItems.keySet()){
            for(var pulseEnchantment : EnchantmentAPI.ReturnCustomEnchantmentOnItems(itemStack)){
                pulseEnchantment.PlayerItemBreakEvent(event, itemStack, inventoryItems.get(itemStack));
            }
        }
        ...
        for(var itemStack : inventoryItems.keySet()){
            ...
            pulseItemStack.PlayerItemBreakEvent(event, itemStack, inventoryItems.get(itemStack));
        }
        ...
        for(var pulseLocation :  LocationAPI.ReturnAllPulseLocations(eventLocation, true)){
            pulseLocation.PlayerItemBreakEvent(event, eventLocation);
        }
    }
}
