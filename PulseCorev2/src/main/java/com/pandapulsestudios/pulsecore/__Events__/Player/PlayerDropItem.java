...
public class PlayerDropItem implements Listener {
    ...
    public void onEvent(PlayerDropItemEvent event){
        ...
        var droppedItemStack = ItemStackAPI.ReturnPulseItem(event.getItemDrop().getItemStack());
        if(droppedItemStack != null){
            if(!event.isCancelled() && droppedItemStack.PlayerDropItemEvent(event, event.getItemDrop().getItemStack(), ItemLocation.DroppedItem)) event.setCancelled(true);
        }
        ...
        for(var itemStack : inventoryItems.keySet()){
            ...
            if(pulseItemStack != null){
                if(!event.isCancelled() && pulseItemStack.PlayerDropItemEvent(event, itemStack, inventoryItems.get(itemStack))) event.setCancelled(true);
            }
        }
        ...
        if(PulseCoreMain.playerActionLock.containsKey(world)) {
            if(!event.isCancelled()) event.setCancelled(PulseCoreMain.playerActionLock.get(world).contains(PlayerAction.PlayerDropItem));
        }
        ...
        for(var coreEvent : PulseCoreMain.pulseCoreEvents){
            if(!event.isCancelled() && coreEvent.PlayerDropItemEvent(event)) event.setCancelled(true);
        }
    }
}
