    ...
    public void OnBlockBreak(PlayerBucketEmptyEvent event){
        for(var pulseCoreEvent : PulseCore.PulseCoreEvents){
            if(pulseCoreEvent.CanDoEvent(event.getPlayer(), event.getPlayer().getLocation())){
                var state = pulseCoreEvent.pulseCoreEvents.PlayerBucketEmptyEvent(event);
        ...
        if(PulseCore.HandlePlayerActionEventsInHouse){
            var state = PlayerAPI.CanDoAction(PlayerAction.PlayerBucketEmpty, (Player) event.getPlayer());
            if(!event.isCancelled()) event.setCancelled(!state);
        }
