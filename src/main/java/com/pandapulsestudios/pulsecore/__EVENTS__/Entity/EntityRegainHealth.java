    ...
    public void onEvent(EntityRegainHealthEvent event){
        ...
        for(var itemStack : inventoryItems.keySet()){
            ...
            for(var nbtListener : PulseCoreMain.nbtListeners){
                if(!event.isCancelled() && nbtListener.EntityRegainHealthEvent(event, itemStack, NBTAPI.GetAll(itemStack), livingEntity)) event.setCancelled(true);
            }

            for(var pulseEnchantment : EnchantmentAPI.ReturnCustomEnchantmentOnItems(itemStack)){
                if(!event.isCancelled() && pulseEnchantment.EntityRegainHealthEvent(event, itemStack, inventoryItems.get(itemStack))) event.setCancelled(true);
            }

            var pulseItemStack = ItemStackAPI.ReturnPulseItem(itemStack);
            if(pulseItemStack != null) if(!event.isCancelled() && pulseItemStack.EntityRegainHealthEvent(event, itemStack, inventoryItems.get(itemStack))) event.setCancelled(true);
        }
        ...
        if(PulseCoreMain.playerActionLock.containsKey(world)  && isEntityPlayer)  if(!event.isCancelled()) event.setCancelled(PulseCoreMain.playerActionLock.get(world).contains(PlayerAction.EntityRegainHealth));
