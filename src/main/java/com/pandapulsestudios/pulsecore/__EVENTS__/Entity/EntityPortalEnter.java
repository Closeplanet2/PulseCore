    ...
    public void onEvent(EntityPortalEnterEvent event){
        ...
        for(var itemStack : inventoryItems.keySet()){
            for(var pulseEnchantment : EnchantmentAPI.ReturnCustomEnchantmentOnItems(itemStack)){
                pulseEnchantment.EntityPortalEnterEvent(event, itemStack, inventoryItems.get(itemStack));
            }
        }
        ...
        for(var itemStack : inventoryItems.keySet()){
            ...
            pulseItemStack.EntityPortalEnterEvent(event, itemStack, inventoryItems.get(itemStack));
        }

        var eventLocation = player.getLocation();
        for(var pulseLocation :  LocationAPI.ReturnAllPulseLocations(eventLocation, true)){
            pulseLocation.EntityPortalEnterEvent(event, eventLocation);
        }
    }
}
