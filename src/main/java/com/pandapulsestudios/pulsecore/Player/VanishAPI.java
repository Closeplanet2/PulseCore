...
public class VanishAPI {
    ...
    public static void HIDE_PLAYER_FROM_PLAYER(Player a, Player b){
        ...
        if(!IS_HIDDEN_FROM_PLAYER(a, b)) PulseCoreMain.HIDE_MATRIX.get(a.getUniqueId()).add(b.getUniqueId());
    }
    ...
    public static void SHOW_PLAYER_TO_PLAYER(Player a, Player b){
        CreateBLankData(a, false);
        PulseCoreMain.HIDE_MATRIX.get(a.getUniqueId()).remove(b.getUniqueId());
    }
    ...
    public static void UPDATE_VANISH(Player player){
        ...
        for(var otherPlayer : Bukkit.getOnlinePlayers()){
            var hiddenPlayers = PulseCoreMain.HIDE_MATRIX.get(player.getUniqueId());
    ...
    public static boolean IS_HIDDEN_FROM_PLAYER(Player a, Player b){
        CreateBLankData(a, false);
        return PulseCoreMain.HIDE_MATRIX.get(a.getUniqueId()).contains(b.getUniqueId());
    }

    private static void CreateBLankData(Player player, boolean override){
        if(!PulseCoreMain.HIDE_MATRIX.containsKey(player.getUniqueId()) || override)
            PulseCoreMain.HIDE_MATRIX.put(player.getUniqueId(), new ArrayList<>());
    }
}
